...
{
    // Form message
    msgBuf = {MQTTPUBLISH + 0x02};

    // Calculate variable length
    uint32_t topicLen = 2 + strlen(topicStr) + 2 + strlen(msgStr);

    // Store the length
    do
    {
        uint8_t lenByte = topicLen % 128;
        topicLen = topicLen / 128;
        if (topicLen > 0)
            lenByte = lenByte | 0x80;
        msgBuf.push_back(lenByte);
    } while (topicLen > 0);
...
{
    // String beforeStr;
    // Raft::getHexStrFromBytes(buf.data(), buf.size(), beforeStr);
    ...
    // String afterStr;
    // Raft::getHexStrFromBytes(buf.data(), buf.size(), afterStr);
