...
void SysManager::setup()
{
    ...
    // pointer to the CommsChannelManager that can be used to send messages
    for (SysModBase* pSysMod : _sysModuleList)
    {
        if (pSysMod)
        {
            ...
            if (_pCommsChannelManager)
                pSysMod->addCommsChannels(*_pCommsChannelManager);
        }            
    }
    ...
    for (SysModBase* pSysMod : _sysModuleList)
    {
        if (pSysMod->modNameStr().equals(sysModName))
        {
            ...
            LOG_I(MODULE_PREFIX, "sendCmdJSON %s rslt %s found in %lldus exec time %lldus", 
                    sysModName, HWElemBase::getRetcStr(rslt), foundSysModUs - startUs, micros() - foundSysModUs);
#endif
            return rslt;
        }
    }
